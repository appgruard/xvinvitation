...
export default function CountdownTimer() {
  ...
  const timerComponents = Object.keys(timeLeft).map((interval) => {
    ...
    return (
      <div key={interval} className="flex flex-col items-center mx-2 md:mx-4">
        <div className="bg-purple-100 w-16 h-16 md:w-24 md:h-24 rounded-lg flex items-center justify-center shadow-inner border border-purple-200 transform rotate-3">
          {/* @ts-ignore */}
          <span className="text-2xl md:text-4xl font-serif text-purple-800 font-bold">{timeLeft[interval]}</span>
        </div>
  ...
  return (
    <div className="py-12 bg-white/50 backdrop-blur-sm rounded-xl mx-4 shadow-lg border border-white">
      <h3 className="text-center font-script text-4xl text-purple-400 mb-8">Faltan...</h3>
      <div className="flex justify-center flex-wrap">
        {timerComponents.length ? timerComponents : <span className="text-3xl font-serif text-purple-800">¡Es hoy!</span>}
      </div>
    </div>
  );
}
